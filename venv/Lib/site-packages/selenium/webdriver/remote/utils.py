...
def unzip_to_temp_dir(zip_file_name):
    ...
    LOGGER.info("Extracting zipped log: %s" % zip_file_name)
    tempdir = tempfile.mkdtemp()

    try:
        # Create directories that don't exist
        for zip_name in zf.namelist():
            # We have no knowledge on the os where the zipped log was
        ...
        for zip_name in zf.namelist():
            ...
            if not (name.endswith(os.path.sep)):
                LOGGER.debug("Copying log %s......" % dest)
                outfile = open(dest, 'wb')
                outfile.write(zf.read(zip_name))
                outfile.close()
                LOGGER.debug("File %s copied." % dest)

        LOGGER.info("Unzipped log can be found at %s" % tempdir)
        return tempdir
